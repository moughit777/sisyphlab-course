    ...
    <section id="curriculum" className="py-24 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-2">
          {course.modules?.map((module, mi) => (
            <motion.div
              ...
              className="rounded-xl overflow-hidden border border-brand-border bg-brand-card"
            >
              {/* Module header */}
              <button
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-brand-green/10 border border-brand-green/20 flex items-center justify-center text-brand-green font-black text-sm">
                    {mi + 1}
                  </div>
                  <div>
                    <div className="font-bold text-brand-white text-sm">{module.title}</div>
                    <div className="text-xs text-brand-muted mt-0.5">
                      {module.lessons?.length} درس · {formatDuration(module.lessons?.reduce((a, l) => a + (l.duration_seconds || 0), 0) || 0)}
                    </div>
                  </div>
                </div>
                  ...
                  >
                    <div className="border-t border-brand-border divide-y divide-brand-border/50">
                      {module.lessons?.map((lesson, li) => (
                        <div key={lesson.id} className="flex items-center gap-3 px-5 py-3.5 text-sm hover:bg-brand-black/40 transition-colors">
                          <Lock className="w-3.5 h-3.5 text-brand-muted flex-shrink-0" />
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-8 p-5 rounded-xl border border-brand-green/15 bg-brand-green/5 text-center"
        >
          <p className="text-brand-gray text-sm mb-4">🔒 جميع الدروس محمية — يمكن الوصول إليها فقط بعد الاشتراك</p>
          <a
            href="#cta"
            className="btn-green inline-block px-6 py-3 rounded-xl text-sm font-bold"
          >
